...
exact_query <- function(bn_fit, evidence_list) {
  mut_bn <- mutilated(bn_fit, evidence = evidence_list)
  grain_mut <- compile(as.grain(mut_bn))
  grain_ev <- setEvidence(grain_mut,
...
ui <- page_navbar(
  ...
  header = tags$head(
    tags$script(HTML("
      Shiny.addCustomMessageHandler('updateToggleStates', function(msg) {
        var selected = msg.selected;
        var behaviors = msg.behaviors;
        behaviors.forEach(function(beh) {
          var el = document.getElementById('imp_' + beh);
          var card = document.getElementById('card_' + beh);
          if (!el || !card) return;
          if (selected && selected !== beh) {
            el.disabled = true;
        ...
        card(
          card_header(
            tags$h5(icon("exchange-alt"),
                    " Select One Behavior to Improve", class = "mb-0"),
            class = "bg-warning text-dark"
          ),
          card_body(
            tags$p("You may toggle only one behavior at a time. Turn it off to select another.",
                   class = "text-muted small mb-3"),
    ...
    layout_column_wrap(
      ...
      card(
        ...
        card_body(
          ...
          tags$p("The model uses the do-operator (graph mutilation) with exact
                  inference via the junction tree algorithm (gRain package)."),
          tags$hr(),
          tags$h6("How to use:"),
          tags$ol(
            ...
            tags$li(tags$strong("Step 2:"), " View your current MASLD probability
                    and simulate the effect of improving one specific behavior.
                    Only one behavior can be toggled at a time; to select
                    another, turn the current one off first.")
          ),
          tags$hr(),
      ...
      card(
        card_header(tags$strong("Network Structure"),
                    class = "bg-primary text-white"),
        card_body(
          tags$img(src = "dag_app.png",
                   style = "width: 100%; height: auto;",
                   alt = "Bayesian Network DAG")
        )
      )
    )
  )
...
server <- function(input, output, session) {
  ...
  observe({
    req(confirmed_evidence())
    ub <- confirmed_unhealthy_display()
    if (length(ub) == 0) return()

    current_sel <- isolate(selected_behavior())
    new_sel <- NULL

    for (beh in ub) {
      val <- input[[paste0("imp_", beh)]]
      if (isTRUE(val)) {
        if (is.null(current_sel) || current_sel == beh) {
          new_sel <- beh
          break
        }
      }
    ...
    if (!identical(current_sel, new_sel)) {
      selected_behavior(new_sel)
    }
  })

  observe({
    sel <- selected_behavior()
    ub <- confirmed_unhealthy_display()
    if (length(ub) == 0) return()

    session$sendCustomMessage("updateToggleStates", list(
      selected = if (is.null(sel)) NULL else sel,
      behaviors = as.list(ub)
    ))
  })
    ...
    tags$div(
      lapply(ub, function(beh) {
        tags$div(
          id = paste0("card_", beh),
          class = "mb-3 p-3 rounded",
          style = "background: #FEF9E7; border: 1px solid #F9E79F; transition: all 0.2s ease;",
          tags$div(
  ...
  improved_result <- reactive({
    ...
    if (is.null(ev_raw) || is.null(p_current) || is.null(sel)) {
      return(list(
    ...
    ev_single[[sel]] <- "0"
    p_single <- exact_query(bn_fit, ev_single)
    eff <- p_current - p_single
    individual_effects[sel] <- max(eff, 0)

    p_improved <- min(p_single, p_current)

    list(p_improved = p_improved,
         individual = individual_effects,
         selected = sel)
  })
  ...
  output$effect_breakdown <- renderUI({
    res <- improved_result()
    sel <- res$selected
    effects <- res$individual

    if (is.null(sel)) {
      return(tags$div(
        class = "text-center py-3 text-muted",
        icon("info-circle", class = "me-1"),
        "Toggle one behavior to see the effect."
      ))
    }

    eff_pp <- effects[sel] * 100
