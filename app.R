...
# Step1: プロファイル入力（確率非表示）
# Step2: 確率表示 + 行動変容シミュレーション
...
# --- モデル読み込み（相対パス — app.Rと同じフォルダにRDataを配置） ---
    ...
    layout_column_wrap(
      ...
      card(
        card_header(tags$strong("Network Structure"),
                    class = "bg-primary text-white"),
        card_body(
          plotOutput("dag_plot", height = "450px")
        )
      )
    )
  )
...
server <- function(input, output, session) {
  ...
  # 矛盾補正: 男性ではSkipping_breakfastを常に"1"で計算
  ...
  output$dag_plot <- renderPlot({
    if (requireNamespace("Rgraphviz", quietly = TRUE) &&
      ...
      label_map_dag <- c(
        "Age"                     = "Age\n(<=50 / >50)",
        "Sex"                     = "Sex",
        "Regular_exercise"        = "Regular\nexercise",
        "Daily_physical_activity" = "Daily physical\nactivity",
        "Walking_speed"           = "Walking\nspeed",
        "Eating_speed"            = "Eating\nspeed",
        "Late_night_eating"       = "Late-night\neating",
        "Skipping_breakfast"      = "Skipping\nbreakfast",
        "MASLD_outcome"           = "MASLD"
      )

      fill_col <- ifelse(nn %in% c("Age", "Sex"), "#3498DB",
                  ifelse(nn == "MASLD_outcome", "#E74C3C", "#18BC9C"))

      nAttrs <- list(
        label     = setNames(label_map_dag[nn], nn),
        fillcolor = setNames(fill_col, nn),
        fontcolor = setNames(rep("white", length(nn)), nn),
        shape     = setNames(rep("ellipse", length(nn)), nn),
        width     = setNames(rep("1.5", length(nn)), nn),
        height    = setNames(rep("0.9", length(nn)), nn)
      )

      attrs <- list(
        node  = list(fontsize = 18, style = "filled", fontname = "Helvetica",
                     penwidth = 2, color = "#2C3E50"),
        edge  = list(color = "#7F8C8D", penwidth = 1.5, arrowsize = 0.8),
        graph = list(rankdir = "TB", ranksep = 1.0, nodesep = 0.5)
      )

      plot(g, nodeAttrs = nAttrs, attrs = attrs)
    } else {
      plot(dag, main = "Bayesian Network DAG")
    }
  })
}
